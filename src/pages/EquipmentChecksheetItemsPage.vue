  ...
  <q-dialog v-model="showDialog" maximized>
    <q-card>
      ...
      <q-card-section class="q-pt-md">
        <!-- Equipment and Checksheet Information Grid -->
        <div class="row q-col-gutter-md">
          ...
          <!-- Equipment Name -->
          <div class="col-12 col-md-3">
            <q-input label="Equipment Name" :model-value="equipment?.name || 'N/A'" outlined dense readonly />
          </div>
          <!-- Activity Code -->
          <div class="col-12 col-md-3">
          ...
          <!-- Tag Number -->
          <div class="col-12 col-md-3">
            <q-input label="Tag Number" :model-value="equipment?.tag_no || 'N/A'" outlined dense readonly />
          </div>
        </div>
    ...
    <ChecksheetHistory v-model="showHistoryDialog" :equipment-id="equipment?.id ?? null"
...
interface ChecksheetItem {
  ...
  status: number; // 0: Action Required, 1: OK, 2: Holding, 3: N/A
  remarks: string | null;
  order: number;
}
...
// Workflow related
const workflowStep = ref<number>(1); // 1: Draft, 2: Completed, 3: Reviewed, 4: Approved
...
const fetchChecksheetItems = async () => {
  if (!props.equipment?.id || !props.checksheet?.id) return;

  loading.value = true;

  try {
    const response = await api.get(
      `/api/equipments/${props.equipment.id}/checksheets/${props.checksheet.id}/checksheet-items`
    );
    ...
    if (status === 'approved') {
      ...
      workflowStep.value = 1; // draft or any other status
    }

    // Debug: Log checksheet data
    console.log('Checksheet prop:', props.checksheet);
    console.log('Checksheet status:', props.checksheet?.status);
    console.log('Initialized workflow step:', workflowStep.value);
    console.log('Assigned technicians:', assignedTechnicians.value);
    console.log('Assigned inspectors:', assignedInspectors.value);
    console.log('Response data technicians:', response.data.technicians);
    console.log('Response data inspectors:', response.data.inspectors);
  } catch (error: unknown) {
...
const fetchTechnicianUsers = async () => {
  ...
  try {
    const response = await api.get(
      `/api/equipments/${props.equipment.id}/checksheets/users-by-role?role=technician`
    );
    technicianUsers.value = response.data.users || [];

    console.log('Fetched technician users:', technicianUsers.value);
...
const fetchInspectorUsers = async () => {
  ...
  try {
    const response = await api.get(
      `/api/equipments/${props.equipment.id}/checksheets/users-by-role?role=inspector`
    );
...
const openAssignTechniciansDialog = async () => {
  ...
  // Fetch technician users if not already loaded
  if (technicianUsers.value.length === 0) {
    await fetchTechnicianUsers();
  }

  // Pre-select currently assigned technicians
...
const assignTechnicians = async () => {
  ...
  try {
    const response = await api.post(`/api/equipments/${props.equipment?.id}/checksheets/${props.checksheet.id}/assign-technicians`, {
      technicians: selectedTechnicians.value,
    });

    // Apply assigned technicians from server response
...
const removeTechnician = (userId: number) => {
  $q.dialog({
    ...
    void (async () => {
      ...
      try {
        const response = await api.post(`/api/equipments/${props.equipment?.id}/checksheets/${props.checksheet.id}/revoke-technicians`, {
          technicians: [userId],
        });
...
const openAssignInspectorsDialog = async () => {
  ...
  // Fetch inspector users if not already loaded
  if (inspectorUsers.value.length === 0) {
    await fetchInspectorUsers();
  }

  // Pre-select currently assigned inspectors
...
const assignInspectors = async () => {
  ...
  try {
    const response = await api.post(`/api/equipments/${props.equipment?.id}/checksheets/${props.checksheet.id}/assign-inspectors`, {
      inspectors: selectedInspectors.value,
    });

    // Apply assigned inspectors from server response
...
const removeInspector = (userId: number) => {
  $q.dialog({
    ...
    void (async () => {
      ...
      try {
        const response = await api.post(`/api/equipments/${props.equipment?.id}/checksheets/${props.checksheet.id}/revoke-inspectors`, {
          inspectors: [userId],
        });
...
const updateDueDate = async () => {
  ...
  try {
    const response = await api.put(
      `/api/equipments/${props.equipment.id}/checksheets/${props.checksheet.id}/updateDueDate`,
      {
    ...
    // Emit event to parent to refresh checksheet list
    emit('statusChanged');
...
const handleFileSelect = () => {
  console.log('Files selected:', uploadFiles.value);
};
...
const uploadPhotos = async () => {
  ...
  try {
    ...
    const response = await api.post(
      `/api/equipments/${props.equipment.id}/checksheets/${props.checksheet.id}/photos`,
      formData,
      {
        headers: {
    ...
    // Add uploaded photos to the list
...
const deletePhoto = (photo: Photo) => {
  $q.dialog({
    ...
    void (async () => {
      ...
      try {
        await api.delete(
          `/api/equipments/${props.equipment.id}/checksheets/${props.checksheet.id}/photos/${photo.id}`
        );
...
const generateReport = async () => {
  ...
  try {
    ...
    const response = await api.get(
      `/api/equipments/${props.equipment.id}/checksheets/${props.checksheet.id}/report`,
      {
        responseType: 'blob',
      }
    );
...
const completeChecksheet = async () => {
  ...
  try {
    await api.put(
      `/api/equipments/${props.equipment.id}/checksheets/${props.checksheet.id}/complete`,
      {
        status: 'completed',
...
const reviewChecksheet = async (approve: boolean) => {
  ...
  try {
    await api.put(
      `/api/equipments/${props.equipment.id}/checksheets/${props.checksheet.id}/review`,
      {
...
const approveChecksheet = async () => {
  ...
  try {
    await api.put(
      `/api/equipments/${props.equipment.id}/checksheets/${props.checksheet.id}/approve`,
      {
        status: 'approved',
      }
    );
...
const rejectChecksheet = () => {
  ...
  $q.dialog({
    ...
    void (async () => {
      ...
      try {
        await api.put(
          `/api/equipments/${props.equipment.id}/checksheets/${props.checksheet.id}/reject`,
          {
            status: 'draft',
          }
        );
...
const generateNextRound = () => {
  ...
  // Verify that the current checksheet is approved
  ...
  $q.dialog({
    ...
    void (async () => {
      ...
      try {
        const response = await api.post(
          `/api/equipments/${props.equipment.id}/checksheets/${props.checksheet.id}/generate-next-round`
        );
