<template>
  <q-page class="q-pa-md">
    <div class="q-mb-md row justify-between items-center">
      <div class="text-h5">Discipline List</div>
      <q-btn color="primary" label="Add Discipline" icon="add" @click="onAdd" />
    </div>

    <q-table :rows="disciplineList" :columns="columns" row-key="id" :loading="loading" v-model:pagination="pagination"
    ...
    <q-dialog v-model="showDialog" persistent>
      <q-card style="min-width: 500px">
        <q-card-section class="bg-primary text-white">
          <div class="text-h6">{{ isEditing ? 'Edit Discipline' : 'Add Discipline' }}</div>
        </q-card-section>

        <q-card-section class="q-pt-md">
          <q-form @submit="onSubmit" class="q-gutter-md">
            <q-input v-model="formData.name" label="Discipline Name *" outlined dense :rules="[
...
const formData = ref<DisciplineFormData>({
  name: '',
  code: '',
  description: '',
  is_active: true,
...
const fetchDisciplineList = async () => {
  ...
  try {
    ...
    disciplineList.value = response.data.disciplines || response.data.data || [];
    pagination.value.rowsNumber = response.data.total || disciplineList.value.length;
    ...
    $q.notify({
      type: 'negative',
      message: 'Failed to load discipline list',
      position: 'bottom',
    });
  } finally {
    loading.value = false;
...
const onSubmit = async () => {
  try {
    if (isEditing.value && editingId.value) {
      await api.put(`/api/disciplines/${editingId.value}`, formData.value);
      $q.notify({
        type: 'positive',
      ...
      await api.post('/api/disciplines', formData.value);
      $q.notify({
        type: 'positive',
    ...
    resetForm();
    await fetchDisciplineList();
  } catch (error: unknown) {
    const err = error as { response?: { data?: { message?: string; errors?: Record<string, string[]> } } };

    let errorMessage = 'Failed to save discipline';
...
const onDelete = (discipline: Discipline) => {
  $q.dialog({
    title: 'Confirm',
    ...
    void (async () => {
      try {
        await api.delete(`/api/disciplines/${discipline.id}`);
        $q.notify({
          type: 'positive',
        ...
        $q.notify({
          type: 'negative',
          message: err.response?.data?.message || 'Failed to delete discipline',
          position: 'bottom',
        });
      }
    })();
